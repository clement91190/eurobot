from mae_generator.mae import MAE, OutState, debugger
...
class MAEPrestart(MAE):
    def __init__(self, com_state_factory):
        ...
        d_dos_cdg = 100 # distance entre le dos et le centre de gravite du robot
       
        #state
        recaly = self.sf.get_recaler()
        set_y0 = self.sf.get_setxycap(Coord(1300, d_dos_cdg, 90)) 
        avance_plac_y = self.sf.get_bf_fw(Coord(600))
        tourne = self.sf.get_bf_cap(Coord(0, 0, 180))
        recalx = self.sf.get_recaler()
        set_x0 = self.sf.get_setxycap(Coord(600, d_dos_cdg, 0))  #TODO change this with real state of the robot 
        out = OutState()
        ...
        set_x0.add_instant_transition(out)

        self.state_list = [
            recaly, set_y0, avance_plac_y, tourne, recalx, set_x0, out]
        self.reinit_state()
